...
######################################################################
# Sanity check in anticipation of using other checksums
######################################################################
if [ "${chars}" -ne "${hashhexbytes}" ]
then
  errecho ${FUNCNAME} ${LINENO} \
    "mismatch in number of characters generated by hash function ${hashexec}"
  exit -1
fi

######################################################################
# This is a hard coded dependencey on a feature of BASH in providing
